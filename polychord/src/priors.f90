module priors_module
    ...
    subroutine create_priors(priors,params,settings)
        ...
        integer, dimension(size(params)) :: sub_cluster_blocks
        integer :: num_prior_blocks
        integer :: num_sub_cluster_blocks
        ...
        ! First determine how many prior blocks and sub cluster blocks there are
        do i_params=1,num_params
            prior_blocks(i_params) = params(i_params)%prior_block
            sub_cluster_blocks(i_params) = params(i_params)%sub_cluster_block
        end do

        ! re-organise these labels
        prior_blocks = relabel(prior_blocks,num_prior_blocks)
        sub_cluster_blocks = relabel(sub_cluster_blocks,num_sub_cluster_blocks,0)

        ! Allocate the priors array
        if(allocated(priors)) deallocate(priors)
        allocate(priors(num_prior_blocks))
        ...
        if(settings%do_clustering) then
            if(allocated(settings%nsub_clustering_sets)) deallocate(settings%nsub_clustering_sets)
            if(allocated(settings%sub_clustering_sets)) deallocate(settings%sub_clustering_sets)

            n = maxval( [(count(sub_cluster_blocks==i),i=1,num_sub_cluster_blocks)])

            allocate(settings%nsub_clustering_sets(num_sub_cluster_blocks))
            allocate(settings%sub_clustering_sets(num_sub_cluster_blocks,n))
            settings%nsub_clustering_sets=0
            settings%sub_clustering_sets=0

            do i_params=1,num_params
                n = sub_cluster_blocks(i_params)
                if(n>0) then
                    settings%nsub_clustering_sets(n) = settings%nsub_clustering_sets(n) +1 
                    settings%sub_clustering_sets(n,settings%nsub_clustering_sets(n)) = hypercube_indices(i_params)
                end if
            end do
        end if
