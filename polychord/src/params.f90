module params_module
    ...
    type param_type
        ...
        integer                   :: sub_cluster_block ! sub cluster block
    ...
    subroutine assign_parameter(param,paramname,latex,speed,prior_type,prior_block,sub_cluster_block,prior_params)
        implicit none
        ...
        integer         ,intent(in) :: sub_cluster_block ! prior block
        ...
        param%sub_cluster_block = sub_cluster_block
        allocate( param%prior_params(size(prior_params)) )
        param%prior_params = prior_params

    end subroutine assign_parameter

    subroutine add_parameter(params,paramname,latex,speed,prior_type,prior_block,sub_cluster_block,prior_params)
        implicit none
        ! Parameter array
        ...
        integer         ,intent(in),optional :: sub_cluster_block ! sub cluster block
        ...
        if(present(speed) .and. present(prior_type) .and. present(prior_block) .and. present(prior_params) .and. present(sub_cluster_block) ) then
            call assign_parameter(params(num_params+1),paramname,latex,speed,prior_type,prior_block,sub_cluster_block,prior_params) 
        else
            call assign_parameter(params(num_params+1),paramname,latex,1,0,0,0,blank_params) 
        end if
