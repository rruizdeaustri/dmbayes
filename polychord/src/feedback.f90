...
module feedback_module
    ...
    subroutine write_opening_statement(settings)
        ...
        if(settings%feedback >=normal_fb) then
            ...
            if(allocated(settings%nsub_clustering_sets).and. settings%do_clustering) then
                if(size(settings%nsub_clustering_sets)==1) then
                    write(stdout_unit,'("Sub clustering on ",I4," set")') size(settings%nsub_clustering_sets)
                else 
                    write(stdout_unit,'("Sub clustering on ",I4," sets")') size(settings%nsub_clustering_sets)
                end if
            end if
...
!rruiz: change I2 by I3
                    if(RTI%logZp(ordering(p))>logzero) then
                        write(stdout_unit,'("log(Z_",I3,")  = ", F8.2, " +/- ", F5.2, " (still evaluating)")') p, logZp(ordering(p)),sqrt(abs(varlogZp(ordering(p))))
                    else
                        write(stdout_unit,'("log(Z_",I3,")  = ? (still evaluating)")') p
                    end if

                else
                    if(RTI%logZp_dead(ordering(p)-RTI%ncluster)>logzero) then
                        write(stdout_unit,'("log(Z_",I3,")  = ", F8.2, " +/- ", F5.2)') p, logZp_dead(ordering(p)-RTI%ncluster),sqrt(abs(varlogZp_dead(ordering(p)-RTI%ncluster)))
                    else
                        write(stdout_unit,'("log(Z_",I3,")  = ?")') p
                    end if
                end if
