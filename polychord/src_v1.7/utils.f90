module utils_module
    ...
    !!
    !! The ignore parameter indicates labels to be 'ignored' and set to zero
    function relabel(array,num_labels,ignore) result(array_relabel)
        implicit none
        ...
        do i_point=1,npoints
            ...
            if(present(ignore)) then
                if( all(array(i_point)/=mapping(1:num_labels)) .and. array(i_point)/=ignore ) then
