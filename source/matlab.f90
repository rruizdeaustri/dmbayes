...
! Module containing plotting routines for matlab (2D and 3D plots)
...
module matlab
  ...
  character (LEN = 80) :: matlab_string = 'SuperBayeS v 1.36'
  ...
  !The parameter below controls the type of colormap used for 2D plots
  integer, parameter :: ShadeStyle = sharp
  !
  real*8 :: caxis_min, caxis_max
  character (LEN = 80) ::  colormap_name 
...
!---------------------------------------------------------
      ...
      if ((together) .and. (D.eq.2)) then
         ...
         write (unit, '(A,I4,A)') 'figure(',ShadeMeanLike,');  clf(''reset'')'
         !write (unit, '(A,I4,A)') 'figure(',ShadeMeanChisq,'); clf(''reset'')'
         ...
         write (unit, '(A)') 'redf = 0.75;'
         write (unit, '(A)') 'bflw = 2.5;'
         write (unit, '(A)') 'bfsize = 25;'
      end if
      write(unit, '(A)') 'lab_fontsize = 24/redf; axes_fontsize = 24/redf; names_fontsize = 10/redf;  cbar_fontsize = 16/redf;  textbox_fontsize = 30/redf; twolines_fontsize = 30/redf;exp_fontsize = 14/redf;  '
...
!---------------------------------------------------------
      ...
      if (ShadeWhat .eq.  ShadeMargProb) then
         ...
         write (aunit, '(A,I4,A,I4,A)') 'scatter(scpts(:,',colix(j2),'),scpts(:,',colix(j),'),''r'', ''o'', ''filled'',''SizeData'', 15);'
      end if
...
!---------------------------------------------------------
      ...
      write(aunit,'(A)') 'text(''String'', ''\it{'//trim(matlab_string)//'}'',''FontSize'',names_fontsize, ''Units'', ''normalized'', ''Position'', [1.0 1.05], ''HorizontalAlignment'', ''right'');'

      gfmt = ''',''FontSize'',lab_fontsize);'
      !labels, axis 
      !xlabel, slightly more sophisticated
      write (aunit,'(a)') ' text(''String'','''//trim(lables(colix(j2)))//''',''FontSize'',lab_fontsize, ''Units'', ''normalized'', ''Position'', [0.5 -0.18], ''HorizontalAlignment'', ''center'');'
      write (aunit,'(a)') 'ylabel('''//trim(lables(colix(j)))//trim(gfmt)
      write (aunit,'(a)') 'axis tight;' 
      !x-axis limits
      if (has_plot_limits(colix(j2))) write (aunit,'(A,2E15.5,A)') 'set(gca,''XLim'', [',  plot_limits_min(colix(j2)),  plot_limits_max(colix(j2)),']);'
      !y-axis limits
      if (has_plot_limits(colix(j))) write (aunit,'(A,2E15.5,A)') 'set(gca,''YLim'', [',  plot_limits_min(colix(j)),  plot_limits_max(colix(j)),']);'
...
!         write (aunit,'(A)') ' set(get(hbar,''xlabel''),''String'', ''Posterior pdf'', ''FontSize'',cbar_fontsize );'
      ...
      !if ((colix(j) .ne. 12+2) .and. (colix(j2) .ne. 30+2)) then
         ...
         if ((.not. together) .or. (plot_num .eq. 1)) then
            write(aunit,'(A)') 'text(''String'', description ,''FontSize'',cbar_fontsize, ''Units'', ''normalized'', ''Position'', [0.97 0.25], ''HorizontalAlignment'', ''right'');'
            
            if (use_log_channels) then
               if (use_BRs) then
                  write (aunit,'(A)') 'description = ''Log BR priors'';'
               else
                  write (aunit,'(A)') 'description = ''Log \sigma_iv priors'';'
               endif
            else
               if (use_BRs) then
                  write (aunit,'(A)') 'description = ''Flat BR priors'';'            
               else
                  write (aunit,'(A)') 'description = ''Flat \sigma_iv priors'';'            
               endif
            end if

            write(aunit,'(A)') 'text(''String'', description ,''FontSize'',cbar_fontsize, ''Units'', ''normalized'', ''Position'', [0.97 0.15], ''HorizontalAlignment'', ''right'');'
         
            write(aunit,'(A)') 'text(''String'', ''CMSSM, \mu>0'' ,''FontSize'',cbar_fontsize, ''Units'', ''normalized'', ''Position'', [0.97 0.05], ''HorizontalAlignment'', ''right'');'
         end if
      !else
         !adds DD limits 
...
!---------------------------------------------------------
      ...
      write (aunit,'(a)')  'colormap(cool); hold on;'
      
      gfmt = ''',''FontSize'',lab_fontsize);'
      !xlabel, slightly more sophisticated
      write (aunit,'(a)') ' text(''String'','''//trim(lables(colix(j)))//''',''FontSize'',lab_fontsize, ''Units'', ''normalized'', ''Position'', [0.5 -0.18], ''HorizontalAlignment'', ''center'');'
      write (aunit,'(a)') 'ylabel(''Probability'', ''FontSize'',lab_fontsize);'
      write (aunit,'(a)') 'axis tight;' 
      !x-axis limits
      if (has_plot_limits(colix(j))) write (aunit,'(A,2E15.5,A)') 'set(gca,''XLim'', [',  plot_limits_min(colix(j)),  plot_limits_max(colix(j)),']);'
      !y-axis limits
      ...
      call AddPointsToGraph(aunit)
      colorbar_on = .false.
      call FinalTouches_SingleFig_MATLAB(60, '_1D_'//trim(IntToStr(plot_num)))
...
!---------------------------------------------------------
      ...
      if (plot_bestfit) then
         write (aunit,'(A)') 'plot(bestfit(1), bestfit(2),''x'',''MarkerSize'',bfsize,''MarkerEdgeColor'',''k'', ''LineWidth'', bflw/redf);'
         write (aunit,'(A)') 'plot(bestfit(1), bestfit(2),''o'',''MarkerSize'',bfsize,''MarkerFaceColor'', ''none'', ''MarkerEdgeColor'',''k'', ''LineWidth'', bflw/redf);'
      end if
      if (plot_mean) write (aunit,'(A)') 'plot(mean(1), mean(2),''o'',''MarkerSize'',bfsize/2,''MarkerFaceColor'',''k'', ''MarkerEdgeColor'',''k'', ''LineWidth'', bflw/redf*0.75);'   
      if (plot_reference) write (aunit,'(A)') 'plot(refpoint(1), refpoint(2),''d'',''MarkerSize'',8,''MarkerFaceColor'',''r'', ''MarkerEdgeColor'',''y'', ''LineWidth'', 2.0);'   
...
!---------------------------------------------------------
      ...
      write(aunit,'(A)') 'ddb1=load (fullfile(''../../data/DD-SI/'',''DDbound_CDMS_update.dat''));'
      write(aunit,'(A)') 'mlsp1=ddb1(:,1)/1000; sigma1=ddb1(:,2); '
      write(aunit,'(A)') 'ddb2=load (fullfile(''../../data/DD-SI/'',''DDbound_ZEPLIN.dat''));'
      write(aunit,'(A)') 'mlsp2=ddb2(:,1)/1000; sigma2=ddb2(:,2)/1.6;' 
      write(aunit,'(A)') 'ddb3=load (fullfile(''../../data/DD-SI/'',''DDbound_EDELWEISS.dat''));'
      write(aunit,'(A)') 'mlsp3=ddb3(:,1)/1000; sigma3=ddb3(:,2); '
      write(aunit,'(A)') 'ddb4=load (fullfile(''../../data/DD-SI/'',''Xe10_136kgd_2007.dat''));'
      ...
      write(aunit,'(A)') 'text(''String'', ''CDMS-II'',''FontSize'',exp_fontsize, ''Units'', ''normalized'', ''Position'', [0.9 0.68],''Color'',''k'',''HorizontalAlignment'', ''right'')' 
      !%XENON10
      write(aunit,'(A)') 'plot(mlsp4*1000,log10(sigma4),''k'', ''Linewidth'',lwdd); '
      write(aunit,'(A)') 'text(''String'', ''XENON-10'',''FontSize'',exp_fontsize, ''Units'', ''normalized'', ''Position'', [0.9 0.85],''Color'',''k'',''HorizontalAlignment'', ''right'')'  
      !%ZEPLIN 
      write(aunit,'(A)') 'plot(mlsp2*1000,log10(sigma2),'':k'', ''Linewidth'',lwdd); '
      write(aunit,'(A)') 'text(''String'', ''ZEPLIN-II'',''FontSize'',exp_fontsize, ''Units'', ''normalized'', ''Position'', [0.4 0.95],''Color'',''k'',''HorizontalAlignment'', ''right'')'  
...
!---------------------------------------------------------
      ...
      if (has_plot_limits(ix1+2)) write (aunit,'(A,2E15.5,A)') 'set(gca,''XLim'', [',  plot_limits_min(ix1+2),  plot_limits_max(ix1+2),']);'
      !y-axis limits
      if (has_plot_limits(ix2+2)) write (aunit,'(A,2E15.5,A)') 'set(gca,''YLim'', [',  plot_limits_min(ix2+2),  plot_limits_max(ix2+2),']);'
      ...
      !fine for most plots
      write (aunit,'(A,I5,A,I5,A,I5,A)') 'scatter(pts(:,',2+ix1,'),pts(:,',2+ix2,'),18,pts(:,',2+ix3,'),''filled'');'
      !bigger dots
      ...
      write (aunit,'(a, 2E15.5,a)') 'mean = [', mean(ix1), mean(ix2), '];'
      write (aunit,'(a, 2E15.5,a)') 'refpoint = [', rf(colix(ix1)-2), rf(colix(ix2)-2), '];'
      !plotted here 
      call AddPointsToGraph(aunit)
      gfmt = ''',''FontSize'',lab_fontsize);'
      !xlabel, slightly more sophisticated
      write (aunit,'(A)') ' text(''String'','''//trim(lables(ix1+2))//''',''FontSize'',lab_fontsize, ''Units'', ''normalized'', ''Position'', [0.5 -0.18], ''HorizontalAlignment'', ''center'');'
      !ylabel
      write (aunit,'(A)') 'ylabel('''//trim(lables(ix2+2))//trim(gfmt)
      write (aunit,'(A)') 'text(''String'', ''\it{'//trim(matlab_string)//'}'',''FontSize'',names_fontsize, ''Units'', ''normalized'', ''Position'', [1.0 1.05], ''HorizontalAlignment'', ''right'');'
      ...
      !colorbar
      write(aunit,'(A)') 'cname  = ''$'//trim(lables(ix3+2))//'$'';'
      ...
      write(aunit,'(A)') 'text(''interpreter'',''latex'',''String'', cname,''FontSize'',axes_fontsize, ''Units'', ''normalized'', ''Position'', [0.5 2.5], ''HorizontalAlignment'', ''center'');'
