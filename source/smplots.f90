...
!
! Module containing plotting routines for SM (1D)
...
module smplots
  ...
  contains
    ...
    subroutine Write1DPlotSM(unit, aroot, j)
      ...
      write (unit,'(A)') 'read {x 1 pdf 2 prof 5}'
      ...
      if (plot_1D_likelihood) then
         ...
         case(22)
            !call Gaussian_1DPlot(unit,DeltaMBs,22)
         case(23)
            !call Gaussian_1DPlot(unit,Butaunu,23)
         case(24) !Higgs limit 
      ...
      if (plot_reference) then
         ...
         write (unit,'(A)') 'ptype 20 0 lweight 5 expand 5 points xrf yrf lweight $lw_st expand $ex_st '        
      end if
      write (unit,'(A)') 'lweight $lw_st '       
    ...
    subroutine Gaussian_1DPlot(unit, datum,j) 
      ...
      if ((datum%tau .ne. 0) .and. (datum%tau_percent .eq. .false.)) then
      ...
      if (.not. has_plot_limits(j+2)) then
          ...
          write (unit,'(A,E15.5,A,E15.5,A,E15.5)') 'set xx = ',plot_limits_min(2+j),',', plot_limits_max(2+j),',',(plot_limits_max(2+j)- plot_limits_min(2+j))/100           
      end if
    ...
    subroutine SmearedBound_1DPlot(unit,sigma, tau, limit ) 
      ...
      write (unit,'(A)') 'set like = 1/sqrt(2*PI*(sigma**2+tau**2))*EXP(-0.5*(x-mstar)**2/(sigma**2+tau**2))*(1- 1/2*erfc(tstar/sqrt(2))) + 1/2*erfc(t2/sqrt(2))' 
      write (unit,'(A)') 'set xx = x'
    ...
    subroutine FinalTouches1DPlotSM(unit)
      ...
      write (unit,'(A)') 'ctype black expand $ex_label'
         ...
         if (use_log_channels) then
            if (use_BRs) then
               write(unit, '(A)') 'putlabel 4 " (Log BR priors)"'
            else
               write(unit, '(A)') 'putlabel 4 " (Log \sigma_iv priors)"'
            endif
         else
            if (use_BRs) then
               write(unit, '(A)') 'putlabel 4 " (Flat BR priors)"'
            else
               write(unit, '(A)') 'putlabel 4 " (Flat \sigma_iv priors)"'
            endif
         end if
  ...
  subroutine WriteSMChainsInit(unit, tag)
    ...
    subroutine  WriteSMLoadChains(unit, aroot)
      ...
      do ic=1, num_chains_used
         ...
         write (unit,'(A)') 'set ch'//trim(adjustl(chstr))//'m = ch'//trim(adjustl(chstr))//'n % 10  set ch'//trim(adjustl(chstr))//'nn = ch'//trim(adjustl(chstr))//'n IF(ch'//trim(adjustl(chstr))//'m == 0)'
    ...
    subroutine ChainPositionPlot(unit, ch, j)
      ...
      write (unit,'(2A)') 'ltype 0 ctype red con ch'//trim(adjustl(chstr))//'nn ch'//trim(adjustl(chstr))//'yy'//trim(adjustl(colstr))
      write (unit,'(A)') 'ltype 0 ctype black'
   ...
   subroutine FinalTouchesChainPlot(unit, ch)
    ...
    subroutine BurnInPlot(unit, ch)
      ...
      integer :: unit, ch
      character(LEN=80) :: colstr, chstr, minstr
      write (unit,'(A)') 'window 1 1 1 1 location 2500 31000 3500 30000'
      write (unit,'(''window'',4I4)') plot_col,plot_row, plot_x, plot_y
      write(chstr, *) ch
