...
program GetPlots
        ...
        character(LEN=30000) InLine, CLine, InLineSM  ! ,LastL,OutLine
        ...
        character(LEN=1000) matlab_col, InS1,InS2,gfmt
        integer :: chain_ix
       
        integer x,y,j,j2, num_2D_plots, num_cust2D_plots
        integer num_3D_plots
        character(LEN=80) plot_3D(20)
        ...
        real counts
        integer :: countres, tot_plots, failures = 0
        ...
        integer :: ii, cc, ps_max

        logical :: IDflag, Ebflag

        character(LEN = 25)::  pname
        character(LEN = 400)::  plname, plot_3D_psname, junk(1:3)
        character(LEN = 5)::   lname(2)

        logical :: NS_files = .false.
        real :: lnevidence, lnev_error
!---------------------------------------------------------------
!                      Read in options
!--------------------------------------------------------------- 
        ...
        use_BRs =  Ini_Read_Logical('use_BRs', .false.)
        use_log_channels =  Ini_Read_Logical('use_log_channels', .false.)
        ...
        do ix = 1, Num_ComparePlots
           ComparePlots(ix) = ExtractFileName(Ini_Read_String(numcat('compare',ix), .true.))
        end do
...
!--------------------------------------------------------------- 
        !allocation
        allocate(bins2D(-1000:1000,-1000:1000),bin2Dlikes(-1000:1000,-1000:1000),bin2Dchisq(-1000:1000,-1000:1000),bin2Dmax(-1000:1000,-1000:1000), bin2D_NDmax(-1000:1000,-1000:1000))
        ...
        do chain_ix = first_chain, first_chain-1 + max(1,chain_num)
           ...
           if (ios .ne. 0) then
              !try chain from NS run
              write(*,*) 'Chain not found!'
              write(*,*) 'Now trying to read ' // trim(infile)//'post.dat'
              open(unit=50,file=trim(infile)//'post.dat',form='formatted',status='old', err=28)
              NS_files = .true.
           end if
...
12            read (50,'(a)',end=1) InLine
              !write(*,*) 'this is the range ',invars(1)
...
!--------------------------------------------------------------- 
       ...
       !Only use variables whose lable's are not empty (and in list of plotparams if plotparams_num /= 0)
        ...
        !there is a factor of 2 in the output of MN wrt MCMC!!!! 
        write (*,'(A, F10.5)')    'Best fit -2 Ln(like) = chisquare      = ', maxlike
        ...
        write (*,'(A, F10.5)')    'Mean(-2 Ln(like)) = mean(chisq) = ', meanchisq
        
        meanlike = -log(sum(exp(-(coldata(2,0:nrows-1) -maxlike))*coldata(1,0:nrows-1)) & 
             / numsamp) + maxlike
           write (*,'(A, F10.5)') '-2 Ln(mean like)                  = ', meanlike
...
!                          Do 1D bins
!------------------------------------------------------------
        write(*,'(A,I6,A,I6)') 'Total number of variables = ', colix(num_vars), ' Used variables =', num_vars
        cont_lines = 0
        plot_num = 0 
        do j = 1,num_vars
           ix = colix(j)
           write(*,'(A,I4,A,I4,A,I2,2A)') 'Now doing 1D var j=', j, ' ix = ', ix,  ' tails = ', tails(ix),  ' name = ', trim(lables(ix))
           bincounts = 0
           binlikes = 0
           binchisq = 0
           binmaxlike = 1e30
           ...
           if (has_limits_bot(ix)) then
              ...
              amin = ConfidVal(ix,0.0001,.false.)
           end if
           if (has_limits_top(ix)) then
              amax = limmax(ix)
           else
              amax = ConfidVal(ix,0.0001,.true.)
           end if

           width(j) = (amax-amin)/(nbins+1)

           !write(*,*) 'widht ', j, width(j)
           if (width(j)==0) cycle
           if (amin <= 0) then 
              center(j) = amax
           else
...
!------------------------------------------------------------------------------------
          ...
          if (num_2D_plots > 0) then
                ...
                if (ix_min(j) /= ix_max(j)) then
                   ...
                   do j2 = j2min, num_vars
                      if (ix_min(j2) /= ix_max(j2)) then
                         ...
                         plot_num = plot_num + 1
                         write(*,'(A,I4,A,3A)') 'Doing 2D plot', plot_num, ' for params ', trim(lables(colix(j))), ' vs ', trim(lables(colix(j2)))
                         done2D(j,j2) = .true.
                         ...
                         !mean likelihood
                         !RT - uncomment the stuff below if you want to plot 
             ...
             if (plot_num < num_cust2D_plots) then
                write(*,'(A,I4,A)') 'WARNING - you have requested', num_cust2D_plots, ' 2D plot(s) '
                write(*,'(A,I4,A)') '          but only', plot_num, ' have been produced'
                write(*,'(A)')      '          You might want to check your choice for plotparams'
             end if
...
!------------------------------------------------------------------------------------
         ...
         if (num_3D_plots /=0) then
            ...
            do j=1, num_3D_plots
               ...
               if (trim(plot_3D_psname) .eq. 'default') then
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         write(50, *) ' '
         write (50,'(A, F15.4,A, F15.4)') 'Average -2 ln(Like)   = ', meanchisq, ' (=  average chisq)'
         write (50,'(A, F15.4,A, F15.4)') '-2 ln(mean like)      = ', meanlike
         write (50,'(A, F15.4,A, F15.4)') 'Best fit -2 ln(like)  = ', maxlike,   ' (= best fit chisq)'
         write (50,'(A, F15.4)')         'Bayesian complexity   = ', (meanchisq - maxlike)
         if (NS_files) then
         ...
         write(50,'(A)') ' parameter                      mean           sddev          lower '//adjustl(lname(1))//'%   upper '//adjustl(lname(1))//'%   lower '//adjustl(lname(2))//'%   upper '//adjustl(lname(2))//'%   tails'
         do j=1, num_vars
         ...
         write(50, '(A)') 'Parameter values bounding the N-dim '//adjustl(lname(1))//'% and '//adjustl(lname(2))//'%  posterior regions'
         write(50, *) ' ' 
         write (50,'(A, F15.5, A)') 'Best fit sample -2 * log(Like) = ',coldata(2,bestfit_ix), '(= chi-square)'
         write (50,*) ' '
         write(50,'(A)') ' parameter                      bestfit        lower '//adjustl(lname(1))//'%   upper '//adjustl(lname(1))//'%   lower '//adjustl(lname(2))//'%   upper '//adjustl(lname(2))//'%   tails'
         do j=1, num_vars
            pname = trim(lables(colix(j)))            
            write(50,'(1I3" '//adjustl(pname)//'", 5E15.6,I4)') & 
                 colix(j)-2, coldata(colix(j),bestfit_ix),&
                 minval(coldata(colix(j),0:ND_cont1)), &
                 maxval(coldata(colix(j),0:ND_cont1)), &
                 minval(coldata(colix(j),0:ND_cont2)), &
                 maxval(coldata(colix(j),0:ND_cont2)), tails(colix(j))
