...
!    call Fermi_Read_PS(Fermi_fixed_PSfile,.true.,Number_fixed_PS,fixed_PS_params)
     ...
     if(.not. Use_Fermi_Simulated) then
        ...
        call Fermi_Read_PS(Fermi_fitme_PSfile,.true.,Number_fixed_PS,fixed_PS_params)
  ...
  SUBROUTINE Initialize_Synthetic_Data(noisy,dims)
    ...
    !PtSrcMap = Fermi_InitPtSrc('./ptsrc_data/gll_psc_v07.fit',RA(1,:),DEC(1,:),PtSrcIndices,dims)
  ...
  integer :: nSrcs, lon_index, lat_index, i, j, k, l_bin, b_bin, m
  ...
  logical :: bad
    ...
    !Use this to add all point sources from catalogue (and comment the whole OPTION2 out)
    !OPTION1 -------------------------------------------------------

! rruiz: to read fake catalog, otherwise reads real data from FITS
...
!       print*,j,GC_Ebins(j,1),GC_Ebins(j,2),binsize,lon_index,lat_index
      enddo
!      close(6)
    enddo
  else 

    !Use this to add mock point sources by hand (and comment the whole OPTION1 out)
    !OPTION2-----------------------------------------------------

        !Implement here the reading stuff

      !call Ini_Open('Follow_up/Test_10PS.txt', 1, bad, .false.)
      ...
      !Read in the number of PS
      nPS = Ini_Read_Int('Number_PS',10)
      allocate(fake_fits(NPtSrcParam,nPS))

      !Read in the PS parameters
      do m = 1,7
       InLine = Ini_Read_String(trim(PSNames(m)))
       read(InLine,*) fake_fits(m,:)
      end do
...
10      EGBGlength = i-1
  ...
  SUBROUTINE Fermi_InitGrid(dims,coords)
    ...
    if (Feedback .gt. 2) write(*,*) 'BG grid dimensions: ', dims(1),dims(2),dims(3),bg_combis,Template_number
  ...
  SUBROUTINE Fermi_readDMSource(DMSourceName, outArray, expArray, DMSource_world, DMSourceDims, E_in, centreLoc, centrePix, pixData)
    ...
    integer, parameter :: triplet(3) = (/1,1,1/)
    character (len=80) :: record, comment, ctype(2), ctype_exp(2)
    character (len=28801) :: header

    !RT: here are the fermi data files 
    ...
    ! Get the observations
      ...
      if (fitstatus .ne. 0) call flatUtils_crash('Could not open observation file '//trim(DMSourceCountsFile)//'.')
...
!      print*,DMSourceDims_in,lowerPix,upperPix
      ! Read the image
      call ftgsvd(unit,0,3,DMSourceDims_in,lowerPix,upperPix,triplet,0,outArray(:,:,2:DMSourceDims(3)-1),any_f,fitstatus)
      ...
      !print*,DMSourceDims(1), DMSourceDims(2), DMSourceDims(3)
