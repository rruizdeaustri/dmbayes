...
!CS: Modified to include the background grid and template parameters, as well as the PS parameters
...
!--------- Constant SM particle masses (GeV) ----------
 ...
 Type (SMMasses), save :: MassData

!--------- Input parameters in SoftSusy ---------
! So far only the CMSSM is implemented 
 Type Input_params
   real*8 :: mx, sigmav, br_bbar, br_ttbar, br_ww, br_zz, br_ee, br_mumu, br_tautau, br_ccbar, br_gg, br_zgam, br_gamgam
 ...
 character(LEN=lmax), dimension(16):: DMNames
 logical, dimension(16) :: DMUsed
  
 Type Nuisance_params
   real (8) :: a0, e0, delta1, delta2, mtop 
 end Type Nuisance_params

 character(LEN=lmax), dimension(5):: NuisanceNames = (/ 'A_0      ', &
                                                        'E_0      ', &
                                                        '\delta_1 ', &
                                                        'delta_2  ', &
                                                        'm_t (GeV)'/)
...
character(LEN=lmax),dimension(2) :: GridNames = (/ 'bg_xs    ', &
                                                   'BG_param2' /)
...
character(LEN=lmax),dimension(9) :: TemplateNames = (/ 'X_CO_1  ', &
                                                       'X_CO_2  ', &
                                                       'X_CO_3  ', &
                                                       'index_e ', &
                                                       'norm_e  ', &
...
!--------- DM parameters ---------------
 ...
 character(LEN=lmax), dimension(2) :: AdditionalOutNames= (/ 'BR_{\tau^+\tau^-}' , &
                                                             'J(\psi = 0)      '/)

!Indirect Detection rates

 Type Gammas_In
   logical :: GCPoissonian
 ...
 Type Gammas_Out
    real (8), dimension(1000) :: fluxgadiff, Ekin
    real (8) :: fluxgac=0d0
 End Type Gammas_Out 
 ...
 End Type ID_Out
  ...
  character(LEN=lmax), dimension(3) :: IDNames= (/ 'd \Phi_\gamma/dE', &
                                                   '\Phi_\gamma     ', &
                                                   'E_{fluxes} (GeV)'/)
