!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
!
!Module for Monte Carlo for MSSM parameters
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
! Contains Monte Carlo sampling routines
! and MSSM specific implementation to compute 
! the MSSM predictions given the input params
...
! 
! Based on the COSMOMC package by Anthony Lewis & Sarah Bridle
! see astro-ph/0205436 and http://cosmologist.info
! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
  ...
  subroutine MCMCsample(Params, accept_num)
    ...
    do while (num_accept <= accept_num+1)
       ...
       if (CurLike /= LogZero .and. slice_sampling) then 
          ...
          if ((num_accept> max(burn_in,1) .and. .not.(just_restarted)) .and. (.not.propose_grid)) then
          ...
          if (.not. propose_grid) then
             ...
             write(*,*) 'After grid proposal:' 
             write(*,*) 'CurParams = ', CurParams%P(1:2)
             write(*,*) 'Trial = ', Trial%P(1:2)
          end if        
          
          write(*,*) 'Computing log like for params = ', Trial%P(1:2)
          Like = GetLogLike(Trial)
          write(*,*) 'After log like call = ', Trial%P(1:2)
          ...
          if (accpt) then
             ...
             if (propose_grid) then
                !for the grid, always write the Trial parameters
                output_lines = output_lines +1
                write(*,*) 'Now writing = ', Trial%P(1:2), Like
                call WriteParams(Trial, 1.0, Like)
             end if
             just_restarted = .false.
             CurLike = Like
             CurParams = Trial
             num_accept = num_accept + 1
             if (propose_grid .and. num_accept.eq.TotGridPoints) exit
       ...
       if (logfile_unit /=0 .and. mod(num,logsteps) == 0) then
          ...
          write (logfile_unit,'(A, F8.2,A, F8.2)') &
               '                :  DM       = ', dm_av, ' +/- ', sqrt(abs(dm_ssq))
          write (logfile_unit,'(A, F8.2,A, F8.2)') &
               '                :  like     = ', like_av, ' +/- ', sqrt(abs(like_ssq))
          write (logfile_unit,'(A, F8.2)') &
               '      Av time per model (s) = ', real((dm_av + like_av)/1000)
          time_per_sample =  real((dm_av + like_av))/1000/(real(num_accept)/num)
          write (logfile_unit,'(A, F8.2)') &
