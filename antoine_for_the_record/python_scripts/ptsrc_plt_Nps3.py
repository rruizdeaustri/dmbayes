...
def true_pts(filename):
  #pts = np.loadtxt(filename, usecols=(0,1,2,3,4,5,6)) #if the .txt file's 1st column contains the PS number, use following line instead
  pts = np.loadtxt(filename, usecols=(1,2,3,4,5,6,7))
  return pts
...
def load_pts(filename):
    ...
    return pts
# AR : removing the first point source for this given run
def usepts_without1(allpts):
	usepts=allpts[np.where(allpts[:,-1] >= 2)]
	usepts[:,-1] = usepts[:,-1] - 1
	return usepts
...
#            print('Generation ' + str(generations[i]) + ', Point source ' + str(ptsrc+1))
        ...
        plt.figure()
        plt.xlabel('Generation')
        plt.ylabel('Best-fit Likelihood - axis starts at 8.e4')
        for ps in range(Nps):
...
def mean_like_evol(allpts, Nps, civ, genlo, genhi, savedir='plots/'):
        ...
        for ps in range(Nps):
          ...
          plt.plot(generations[1:npts], fracPS[:,ps], label= 'PS ' +str(ps+1))
...
def plot_cnvg(allpts,Nps,civ,genlo,genhi,savedir='plots/'):
        ...
        for i in range(npts):
          ...
          #AR : test
          if (i > 3 ):
            print('i = ' + str(i) + '     meanpts[i] = ' + str(meanpts[i]))
            #'     genpts[:,0] = '+ str(genpts[:,0]) +
          for ps in range(Nps):
        ...
        for col in (Nps,Nps+1):
          ...
          #AR : test
#          print('generations[10:] = ' + str(generations[10:]))
#          print('plotpts[:,col] = ' + str(plotpts[:,col]))
#          print('col = ' + str(col) +'len(generations[10:]) = ' + str(len(generations[10:])) + ' *** ' + 'len(plotpts[:,col])  = ' + str(len(plotpts[:,col])))
          plt.plot(generations[10:], plotpts[:,col], linecolours[col-Nps], label=pltlabels[col-Nps])

        plt.yscale('log')
        plt.xlim(genlo+1, genhi)
        #AR : added this ylim to search for lost points
        #plt.ylim(10**(-6.),100)
        plt.legend(loc=1)
...
def scplot(allpts, truevals, ptsrc, civ, genlo, genhi, save=False, savedir='plots/'):
   ...
   # determine the names of parameter & label plot
    ...
    #Plot true point
        trupt=ax.scatter(truevals[ptsrc-1,param1-3], truevals[ptsrc-1,param2-3], s=120, c='y', marker='^', edgecolors='r')
    ...
    # minlocs is an array (best point occurs in successive generations)
        ...
        bstfitpt=ax.scatter(xmin[0], ymin[0], s=120, c='r', marker='^',edgecolors='cyan')
    ...
    #add legend
    fig.legend((trupt,bstfitpt),('true point','best fit'),scatterpoints=1,prop={'size':8})
...
def plot_spectra(allpts, truevals, ptsrc, civ, genlo, genhi, save=False, savedir='/plots/'):
        ...
        for i in range(np.size(E)):
          ...
          dNdE_true[i] = dNdE(E[i], truevals[ptsrc-1][2], truevals[ptsrc-1][3], truevals[ptsrc-1][4], truevals[ptsrc-1][5], truevals[ptsrc-1][6])
