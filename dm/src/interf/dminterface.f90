subroutine darkmatter(Flag, InP, InN, DMInOut)
  ...
  real (8) :: dmhmj, dmhmjave, dmhrgacontdiff, dmhrgacont, dmhrgacdiffsusy ! ID functions
  ...
  ! Fermi declarations
  integer, parameter :: njsamples = 50
  integer, parameter :: jsamples_power = 2
    ...
    if (Flag%ID_Flags_gamma%GC_region) then
      ...
      if(Flag%use_data == 1) then   !fermi data
        ...
        do j=1,DMInOut%ID_in%gammas%GCDims(3)
          ...
          !Differential continuum/IB spectrum at bin centre
          GC_susyBits(j) = dmhrgacdiffsusy(DMInOut%ID_in%gammas%GC_Ebins(j,3),istat)

          !Average continuum/IB spectrum over full bin width
          ...
          !Addition of line spectra (smeared over a single bin)
